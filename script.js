...
// Store the last values so updating one room doesn't overwrite the other.
// field8 format: arduinoR,arduinoG,arduinoB,m5R,m5G,m5B
// Room 1 = Arduino  -> first 3 values
// Room 2 = M5Stack  -> last 3 values
var lastRgbArduino = '255,0,0'; // matches your HTML initial Room 1 (#FF0000)
var lastRgbM5Stack = '0,255,0'; // matches your HTML initial Room 2 (#00FF00)
...
function applyColor(room) {
  ...
  if(picker) {
    ...
    // Update only the targeted room's stored RGB
    if (room === 1) {
      // Room 1 = Arduino => first 3
      lastRgbArduino = rgbStr;
    } else if (room === 2) {
      // Room 2 = M5Stack => last 3
      lastRgbM5Stack = rgbStr;
    }

    // Always send combined payload in field8:
    // arduinoR,arduinoG,arduinoB,m5R,m5G,m5B
